<template>
  <div class="deal_wrap" >
    <van-nav-bar :placeholder="true" fixed>
      ...
      <template #left>
        <div @click="test">
          ...
          <van-icon
          :name="require('../assets/images/common/deal_title_more.png')"
          size="18"
        />
        </div>
      </template>
    ...
    <div class="content">
      <div class="con_left">
        <div class="title">
          <div @click="tradType = 1" :class="['bay', { active: tradType == 1 }]">
           {{$t('base.bay')}}
          </div>
          <!-- <div
            :class="[
          ...
          ></div> -->
          <div @click="tradType = 2" :class="['sell', { active: tradType == 2 }]">
            {{$t('base.sell')}}
          </div>
        </div>
        ...
        <div :class="[{'sell':tradType == 1 },{'bay':tradType == 2 } ,'txt']">
          ...
          <div class="in">
            <input type="text" v-model="Quantity" @blur="changeQuantity" :placeholder="$t('base.quantity')"/>
          </div>
          <div class="add" @click="crtQuantity('add')"></div>
        </div>
        <div class="bot">
          <p class="tips" v-show="tradType == 1"> <span>{{$t('deal.usable')}}:</span> <span> {{CombinationSymbols.QuoteCurrencyBalance }} USDT</span></p>
          <p class="tips" v-show="tradType == 2"><span>{{$t('deal.usable')}}:</span><span>{{CombinationSymbols.BaseCurrencyBalance}} {{this.BaseCurrency}}</span></p>
          <ul :class="[{'sell':tradType == 1 },{'bay':tradType == 2 } ]">
            <li @click="clickQuantity(index)" :class="{'active':index == QuantityIndex}" v-for=" (i, index ) in quantityLinst">
              {{i.label}}
            </li>
          </ul>
          <div>
            <button :class="['main_btn', {'sell':tradType ==1},{'bay' :tradType==2}]" @click="showPwd">
              {{(tradType ==1?$t('base.bay'):$t('base.sell'))+'   '+BaseCurrency}}
            </button>
          </div>
        </div>
      </div>
      <div class="con_right">
       <ul class="title">
        ...
        <p class="sub-color">≈{{SymbolsMarket.ClosePriceCNY | decimals(2)}} USD</p>
    ...
    <div class="record">
      <div class="title">
        <span>{{$t("deal.entrust")}}</span>
        <span @click="toHistory"><img src="../assets/images/common/deal_record.png" alt=""> {{$t("deal.history")}}</span>
      </div>
      <div class="con">
        <van-empty v-show="isEmpty" :description="$t('base.noData')"    class="custom-image" :image="emptyImg" />

        <div class="card" v-for=" i in OrdersPageList">
            <div>
              <p :class="['tag' ,{'bay':i.category == 1},{'sell':i.category == 2}]">{{i.category==1?$t('base.bay'):$t("base.sell")}}</p>
              <p class="time"> {{$t('deal.createdate')}} : {{$moment(i.createdate).format('YYYY-MM-DD HH:mm:ss')}}</p>
              <p> <span>{{$t('deal.category')}} : {{i.basecurrency}}/{{i.quotecurrency}}</span>  </p>
              <p> <span>{{$t('deal.price')}} : {{i.price}}</span> </p>
              <p>{{$t('base.quantity')}} : {{i.quantity}}</p>
            </div>
            <div>
                <button class="main_btn" @click="cancelOrder(i.delegateordersid)">{{$t('deal.cancel')}}</button>
            </div>
        </div>
        <!-- <div class="card">
            <div>
    ...
    <mainFooter />
        <van-dialog v-model="show" :title="$t('login.password')"  @confirm='submitDelegateOrders' show-cancel-button>
...
import {SubmitDelegateOrders,GetSymbolsMarketDepth,GetCombinationSymbolsEntity,GetDelegateOrdersPageList,CancelDelegateOrders} from '@/api';
...
export default {
  ...
  mounted() {
    ...
    let SymbolsIdTiem = localStorage.getItem("SymbolsIdTiem")?JSON.parse(localStorage.getItem("SymbolsIdTiem")):'';
    ...
    crtPrice(type){
      ...
      if(this.QuantityIndex != -1){
        if(this.tradType == 1 ){
              ...
              let q = ((this.CombinationSymbols.QuoteCurrencyBalance*num)/this.Price);
