<template>
  <div class="register_wrap">
      <!-- :title="$t('login.title')" -->
    <van-nav-bar  :right-text="$t('login.title')"  left-arrow @click-left="$router.go(-1)"  @click-right="$router.push('/login')" :placeholder="true"  fixed >
    <template #right>
    ...
    <downLoad  @closeDownLoad='closeDownLoad' v-show="showDownLoad"/>
      ...
      <div class="txt_box">
         ...
         <div class="item" v-show="navIndex==1">
             <input type="email"  v-model="Email" :placeholder="$t('register.email')" ref="Email"><b v-show="Email" @click="delTxt('Email')"></b>
         </div>
         ...
         <div class="item pwd">
            <input type="password" :placeholder="$t('register.password1')" v-model="Password1" ref="Password1"><b v-show="Password1" @click="delTxt('Password1')"></b>
         </div>
         <div class="item initcode">
            <input type="text" :readonly='isShare' :placeholder="$t('register.recommend')" v-model="RecommendEnCode" ref="RecommendEnCode"><b v-show="RecommendEnCode && !isShare" @click="delTxt('RecommendEnCode')"></b>
         </div>
      </div>
      <div class="bot">
        <div class="btns">
            ...
            <div class="agree">
           <label @click="checked = !checked"> <b :class="{active:checked}" > </b> {{$t('register.agree')}} </label>  <p @click="toH5" >{{$t('register.agreement')}}</p>  
          </div>
            <!-- <div class="login">
      ...
      getCode(){
        ...
        sign(){
            let data = {
                ...
                RecommendEnCode:this.RecommendEnCode?this.RecommendEnCode:'Default',//this.RecommendEnCode,//邀请码（选填）</br>
        ...
        toH5(){
            ...
            let url = this.$config.imgPath+`/News/AppSynopsis/8f14ea47-5fce-4741-aee8-4d85eeb7a69c?LanguageVersion=${lang}`
            this.$router.push({
                path:'/H5',
                query:{
                    url:url,
