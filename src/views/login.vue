<template>
  <div class="login_wrap" ref="login_wrap">
    <!-- <van-nav-bar :title="$t('login.title')" left-arrow  left-text  :right-text="$t('login.rightTitle')"  @click-right="$router.push('/register')" @click-left="$router.replace('/')" /> -->
      <div class="logo">
           <p class="reg"><img @click="$router.push('/')"  src="../assets/images/common/Nav_close.png" alt=""> <span  @click="$router.push('/register')" >{{$t("login.Registernow")}}</span></p>
          <!-- <div class="pic">
      ...
      <div class="txt_box" >
           ...
           <span @click="showSelPhoneAre=true">+{{PhoneAre}}</span>  <input type="text" :placeholder="$t('login.phone')" v-model="Phone" ref="Phone"><b v-show="Phone"  @click="delTxt('Phone')"></b>
         </div>
         <div class="item " v-show="navIndex==1">
             <input type="text" :placeholder="$t('login.email')" v-model="Email" ref="Email"><b v-show="Email"  @click="delTxt('Email')"></b>
         </div>
         <div class="item ">
             <input type="password" :placeholder="$t('login.password')" v-model="Password" ref="Password"><b v-show="Password"  @click="delTxt('Password')"></b>
         </div>
      </div>
    
      <div class="bot">
        <div class="btns">
            <button :disabled=' !(Password && (navIndex==1?Email:Phone))' class="main_btn" @click="login">
                {{$t('login.btnLogin')}}
            </button>
        </div>
        <div class="nav">
            <span @click="$router.push('/set_lan')">{{$t('login.changeLan')}}</span>
            <span @click="$router.push('/forget_pwd')">{{$t('login.forgetPwd')}}</span>
        </div>
      </div>
      <div class="tips">
  ...
  methods:{
      login(){
        let data = {
            ...
            Account:this.navIndex==1?this.Email.trim():(this.PhoneAre+"|"+this.Phone).trim(),
        ...
        Login(data).then(r=>{
            ...
            if(data.Code == 200 ){
                let that = this;
                 that.getEntityUser(data.Data.UserId).then(
                     _=>{
                         this.$toast(
                         {
                            duration: 1500,
